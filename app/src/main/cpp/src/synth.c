...
int sp_synth_computef(sp_data *sp, sp_synth *synth, int bufsize, SPFLOAT *buf)
{
    ...
    for(i = 0; i < bufsize ; i++) {
        ...
        tmp[2] *= 0.3;
        tmp[5] = tmp[5] + tmp[2];
        sp_delay_compute(sp, synth->delay, &tmp[5], &tmp[0]);
        sp_butlp_compute(sp, synth->lpf, &tmp[0], &tmp[6]);
        tmp[6] *= 0.9;
        tmp[5] = tmp[5] + tmp[6];
        tmp[5] = tmp[5] + tmp[4];
